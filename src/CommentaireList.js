import React, { Component } from "react";
import axios from "axios";

//J'ai fais ca a l'aveugle car l'url ne marche pas j'ai un "Not Found" sur internet
//https://supdevinci.nine1000.tech/comments alors qu'avec posts ou authors ca marche
//https://supdevinci.nine1000.tech/posts et https://supdevinci.nine1000.tech/authors
//J'ai ajoutés un dossier avec les screen dans le rendu

class CommentaireList extends Component {
  state = {
    comments: {
      count: null,
      id: null,
      content: null,
      title: null,
      author: null,
      created_at: null,
      result: [],
    },
  };

  deleteclick(s) {
    const headers = {
      "x-token":
        "",
    };
    axios
      .delete("https://supdevinci.nine1000.tech/comments/" + s, { headers })
      .then((response) => console.log(response))
      .catch(function (error) {
        console.log(error);
      });
    window.location.reload();
  }

  componentDidMount() {
    fetch("https://supdevinci.nine1000.tech/comments?limit=5")
      .then((response) => response.json())
      .then((rs) => this.setState({ posts: rs }))
      .catch(function (error) {
        console.log(error);
      });
  }
  getPagenatedData(i) {
    var url = "https://supdevinci.nine1000.tech/comments?limit=5&offset=" + i;
    fetch(url)
      .then((response) => response.json())
      .then((rs) => this.setState({ posts: rs }))
      .catch(function (error) {
        console.log(error);
      });
  }

  render() {
    return (
      <div>
        <div className="container">
          <center>
            <h1>Posts count = {this.state.comments.count}</h1>
          </center>
          <center>
            <h1>Posts Listes</h1>
          </center>
          <a href="CreatePosts">
            Ajouter un post
            <br />
          </a>
          <a href="GestionPosts">
            Supprimer/Modifer un post
            <br />
          </a>
        </div>
        {this.state.comments.result.map((comment) => (
          <div className="container">
            <div>
              <a
                href="OpenPosts"
                onClick={() => this.getlienclick(comment.id)}
                className="title"
              >
                {comment.title}
              </a>
              <h4>{comment.created_at}</h4>
              <p>{comment.content}</p>
            </div>
          </div>
        ))}
        <div className="container">
          <li>
            <ul>
              <h7>
                Pagination :<br />
              </h7>
              {[
                ...Array(
                  this.state.comments.count % 5 === 0
                    ? Math.floor(this.state.comments.count / 5)
                    : Math.floor(this.state.comments.count / 2) + 1
                ).keys(),
              ].map((i) => (
                <a
                  className="page"
                  onClick={() => this.getPagenatedData(i + 1)}
                >
                  -{i + 1}-
                </a>
              ))}
            </ul>
          </li>
        </div>
      </div>
    );
  }
}

export default CommentaireList;
